...
<div class="card">
    ...
    <div class="card-body">
      <form method="post" action="{{route('product.update',$product->id)}}">
        ...
        <div class="form-group">
          <label for="size">Ukuran (Panjang x Lebar x Tinggi dalam cm)</label>
          <input type="text" name="size" class="form-control" placeholder="Contoh: 120 x 60 x 75" value="{{$product->size}}" class="form-control">
          @error('size')
          <span class="text-danger">{{$message}}</span>
          @enderror
        </div>
        ...
        <div class="form-group">
          ...
          <select id="material-select" name="material" class="form-control" onchange="toggleCustomMaterialInput(this)">
              <option value="default" {{ $product->material == 'default' ? 'selected' : '' }}>Default</option>
              <option value="Jati" {{ $product->material == 'Jati' ? 'selected' : '' }}>Jati</option>
              <option value="Mahoni" {{ $product->material == 'Mahoni' ? 'selected' : '' }}>Mahoni</option>
              <option value="Randu" {{ $product->material == 'Randu' ? 'selected' : '' }}>Randu</option>
              <option value="custom" {{ !in_array($product->material, ['default', 'Jati', 'Mahoni', 'Randu']) ? 'selected' : '' }}>Lainnya...</option>
          </select>
          
          @php
                        $customMaterial = !in_array($product->material, ['default', 'Jati', 'Mahoni', 'Randu'])
                            ? $product->material
                            : '';
                    @endphp
                    <input type="text" id="custom-material" name="custom_material" class="form-control mt-2"
                        placeholder="Masukkan bahan lain"
